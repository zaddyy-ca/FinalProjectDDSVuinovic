...
preppedData = fullData %>% select(-YearsInCurrentRole, -Over18, -OverTime, -StandardHours, 
                                  ...
                                  -YearsWithCurrManager, -NumCompaniesWorked, -Education, -DistanceFromHome, -YearsSinceLastPromotion, 
...
#preppedData$DistanceFromHomeGroup = ifelse(preppedData$DistanceFromHome <= 5, "Near", 
                                              # ifelse(preppedData$DistanceFromHome <= 15, "Medium", "Far"))
...
predictions_knn <- predict(model_knn, test_data[, -ncol(test_data)])

# Compare the predicted values with the actual values
table(predictions_knn, test_data$Attrition)

confusionMatrix(predictions_knn, test_data$Attrition)

test_data$Results = predictions_nb

ggplot(test_data, aes(x = YearsAtCompany, fill = Results)) + 
  geom_bar(position = "dodge") + 
  scale_fill_manual(values = c("yes" = "green", "no" = "red")) + 
  ggtitle("College Attendance by Year") + 
  xlab("Year") + 
  ylab("Count")
```
...
ggplot(test_data, aes(x = DistanceFromHome, fill = results)) + 
  geom_bar(position = "dodge") + 
